...
class UserForm(tk.Toplevel):
    ...
    def on_closing(self):
        if mbox.askyesno(
            "",
            "Необходимо ввести данные для подключения, уверен, что ты нас покидаешь?",
        ):
            self.destroy()
    ...
    def entries(self):
        ...
        for entry in entries_list:
            entries.append(
                tk.Entry(self, textvariable=entry, bg='white', fg='black')
            )
            entries[-1].grid(row=row, column=column)
            row += 1
            entries[-1].config(insertbackground='black')
        entries[0].focus_set()
        tk.Button(self, text="Ввести данные", command=self.destroy).grid(
            row=5, columnspan=2
        )
